...
title "=== Git 历史清空工具（支持私有仓库 & 自动检测默认分支 & 批量操作） ==="
...
while true; do
    ...
    for INPUT_REPO in "${REPO_LIST[@]}"; do
        ...
        if git show-ref --verify --quiet refs/remotes/origin/HEAD; then
            ...
            HEAD_INFO=$(git remote show origin -n 2>/dev/null | grep 'HEAD branch' | awk '{print $NF}')
            if [[ -n "$HEAD_INFO" && "$HEAD_INFO" != "(unknown)" ]]; then
                DEFAULT_BRANCH="$HEAD_INFO"
            fi
        fi
