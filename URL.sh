...
async function handleRequest(request) {
  ...
  if (request.method === "POST" && pathname === "/") {
    ...
    if (!contentType.includes("application/json")) {
      return new Response(JSON.stringify({ error: "仅支持 application/json" }), { status: 415, headers: jsonHeaders });
    }

    // 限制请求体大小（4KB）
    const contentLength = Number(request.headers.get("content-length") || 0);
    if (!Number.isFinite(contentLength) || contentLength < 0) {
      return new Response(JSON.stringify({ error: "非法请求" }), { status: 400, headers: jsonHeaders });
    }
    if (contentLength > 4096) {
      return new Response(JSON.stringify({ error: "请求体过大" }), { status: 413, headers: jsonHeaders });
    }
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: "无效 JSON" }), { status: 400, headers: jsonHeaders });
    }

    if (!req || typeof req !== "object" || Array.isArray(req)) {
      return new Response(JSON.stringify({ error: "请求格式错误" }), { status: 400, headers: jsonHeaders });
    }
    ...
    if (!reqUrl) {
      return new Response(JSON.stringify({ error: "链接不能为空" }), { status: 400, headers: jsonHeaders });
    }
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: "链接格式错误" }), { status: 400, headers: jsonHeaders });
    }

    if (urlObj.protocol !== "http:" && urlObj.protocol !== "https:") {
      return new Response(JSON.stringify({ error: "仅支持 http/https 链接" }), { status: 400, headers: jsonHeaders });
    }

    const host = String(urlObj.hostname || "").toLowerCase();
    if (domainBlacklist.length && isHostBlocked(host, domainBlacklist)) {
      return new Response(JSON.stringify({ error: ERR_DOMAIN_BLOCKED }), { status: 403, headers: jsonHeaders });
    }
    ...
    if (customKey && isSuffixBlocked(customKey, suffixBlacklist)) {
      return new Response(JSON.stringify({ error: ERR_SUFFIX_TAKEN }), { status: 409, headers: jsonHeaders });
    }
    ...
    if (captchaEnabled === "true") {
      if (typeof req.cf_token !== "string" || !req.cf_token.trim()) {
        return new Response(JSON.stringify({ error: "缺少人机验证" }), { status: 403, headers: jsonHeaders });
      }

      const secret = typeof TURNSTILE_SECRET_KEY === "string" ? TURNSTILE_SECRET_KEY : "";
      if (!secret) {
        return new Response(JSON.stringify({ error: "验证码服务未配置" }), { status: 500, headers: jsonHeaders });
      }
      ...
      try {
        ...
        if (!verifyData.success) {
          return new Response(JSON.stringify({ error: "人机验证失败" }), { status: 403, headers: jsonHeaders });
        }
      } catch (e) {
        return new Response(JSON.stringify({ error: "验证服务异常" }), { status: 500, headers: jsonHeaders });
      }
    }
    ...
    if (!customKey && suffixBlacklist.length) {
      ...
      if (isSuffixBlocked(key, suffixBlacklist)) {
        return new Response(JSON.stringify({ error: "生成失败，请稍后重试" }), { status: 500, headers: jsonHeaders });
      }
    }
    ...
    return new Response(JSON.stringify({ key: "/" + key }), { headers: jsonHeaders });
  }
