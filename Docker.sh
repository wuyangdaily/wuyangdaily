...
build_service_run_cmd() {
    ...
    case "$service" in
        npm)
            printf 'docker run -d --name %q --restart=always -p 80:80 -p 81:81 -p 443:443 -e TZ=Asia/Shanghai -v %q:/data -v %q:/etc/letsencrypt %q' \
                "$container_name" \
                "${data_dir}/data" \
    ...
    while true; do
        ...
        case $opt in
            ...
            4)
                ...
                ;;
            5)
                selected=$(select_containers multi "📦 请选择要停止的容器：") || { pause_return; continue; }
                ...
                ;;
            6)
                selected=$(select_containers multi "📦 请选择要重启的容器：") || { pause_return; continue; }
                for container in $selected; do
...
install_npm() {
    ...
    save_service_config "npm" "$image" "$npm_base" "-p 80:80 -p 81:81 -p 443:443 -e TZ=Asia/Shanghai -v ${npm_base}/data:/data -v ${npm_base}/letsencrypt:/etc/letsencrypt" || true
...
update_caddy() {
    ...
    local OLD_VERSION
    OLD_VERSION=$(caddy version 2>/dev/null | head -n1)
    ...
    local VERSION
    VERSION=$(curl -fsSL https://api.github.com/repos/caddyserver/caddy/releases/latest | jq -r '.tag_name')
    ...
    local TMP_FILE
    TMP_FILE=$(mktemp)
    curl -fsSL "https://github.com/caddyserver/caddy/releases/download/${VERSION}/caddy_${VERSION#v}_linux_${ARCH}.tar.gz" -o "$TMP_FILE"
    if [ $? -ne 0 ]; then
        echo "⚠️ 下载失败"
        rm -f "$TMP_FILE"
        systemctl start caddy 2>/dev/null || true
