...
update_docker() {
    ...
    if [[ "$OS" =~ ^(ubuntu|debian|armbian)$ ]]; then
        ...
        for src in "${SOURCES[@]}"; do
            ...
            if apt update 2>&1 | tee /tmp/apt_update.log; then
                if grep -qiE "Could not handshake|Failed to fetch|Err:[[:space:]]*$src|Ign:[[:space:]]*$src.*Connection" /tmp/apt_update.log; then
                    echo "⚠️ 源 $src 连接失败，尝试下一个"
                    continue
                fi
                ...
                if grep -q "NO_PUBKEY" /tmp/apt_update.log; then
                    ...
                    if [ -s /etc/apt/keyrings/docker.gpg ]; then
                        echo "✅ 公钥导入成功，重试更新..."
                        if apt update && DEBIAN_FRONTEND=noninteractive apt upgrade -y -qq 2>/dev/null; then
                            if apt-cache madison docker-ce 2>/dev/null | grep -qF "$src"; then
                                echo -e "\n🧹 自动清理不再需要的依赖包..."
                                apt autoremove -y -qq 2>/dev/null || true
                                echo -e "\n✅ 成功从源 $src 更新系统及 Docker"
                                ...
                                echo "⚠️ 公钥修复后，源 $src 仍未提供 docker-ce，尝试下一个"
                            fi
                        else
                            echo "⚠️ 公钥修复后更新仍然失败，尝试下一个"
                        fi
                    else
                        echo "⚠️ 公钥修复失败，尝试下一个"
                    fi
                else
                    echo "⚠️ apt update 失败，非公钥问题，尝试下一个"
                fi
            fi
        done
        rm -f /tmp/apt_update.log
        ...
        for src in "${SOURCES[@]}"; do
            ...
            if command -v dnf >/dev/null 2>&1; then
                ...
                if ! dnf makecache -y 2>/dev/null; then
                    echo "⚠️ 源 $src makecache 失败，尝试下一个"
                    continue
                fi
            else
                echo "📦 使用 yum 生成缓存..."
                if ! yum makecache -y 2>/dev/null; then
                    echo "⚠️ 源 $src makecache 失败，尝试下一个"
                    continue
                fi
            fi
            ...
            if command -v dnf >/dev/null 2>&1; then
                if ! dnf list docker-ce &>/dev/null; then
                    echo "⚠️ 源 $src 未提供 docker-ce 包，尝试下一个"
                    continue
                fi
            else
                if ! yum list docker-ce &>/dev/null; then
                    echo "⚠️ 源 $src 未提供 docker-ce 包，尝试下一个"
                    continue
                fi
            fi
...
view_container_logs() {
    ...
    local container
    container=$(select_containers single "📦 请选择要查看日志的容器") || return
    [ -z "$container" ] && return
    echo "请选择日志查看方式："
    echo "1) 最后 50 行"
    echo "2) 最后 100 行"
...
export_container_start_cmd() {
    ...
    local container=$(select_containers single "📦 请选择要导出命令的容器：") || return
    [ -z "$container" ] && return
...
browse_container_files() {
    ...
    local container
    container=$(select_containers single "📦 请选择要浏览文件的容器：") || return
    [ -z "$container" ] && return
...
container_network_menu() {
    while true; do
        ...
        case $net_opt in
            ...
            5)
                local container
                container=$(select_containers single "📦 请选择要连接网络的容器：") || continue
            ...
            6)
                local container
                container=$(select_containers single "📦 请选择要断开网络的容器：") || continue
                [ -z "$container" ] && continue
