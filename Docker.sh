...
infer_data_dir_from_container() {
    ...
    case "$service" in
        ...
        *)
            local first_bind
            first_bind=$(jq -r '.[0].Mounts[]? | select(.Type=="bind") | .Source' "$inspect_file" | head -n1)
...
update_docker_service() {
    ...
    if [ -n "$image_keyword" ]; then
        ...
        actual_container=$(docker ps -a --format "{{.Names}} {{.Image}}" | grep -i "$image_keyword" | head -n1 | awk '{print $1}')
    fi
    ...
    if [ -z "$actual_container" ]; then
        echo "❌ 未找到 $display_name 的安装配置，且没有检测到运行中的容器。请先安装。"
        return 1
    fi
...
echo "=============================="
    ...
    case "$OS" in
        ubuntu|debian|armbian)
            ...
            curl -fsSL "$DOCKER_SRC/gpg" | gpg --dearmor -o /etc/apt/keyrings/docker.gpg
            echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] $DOCKER_SRC $(lsb_release -cs) stable" > /etc/apt/sources.list.d/docker.list
            apt update
        ...
        centos|rocky|fedora|rhel)
            if command -v dnf >/dev/null 2>&1; then
                ...
                dnf config-manager --add-repo "$DOCKER_SRC/docker-ce.repo"
                dnf -y install docker-ce docker-ce-cli containerd.io docker-compose-plugin
            else
                yum -y install yum-utils
                yum-config-manager --add-repo "$DOCKER_SRC/docker-ce.repo"
...
update_docker() {
    ...
    echo "✅ Docker 版本: $DOCKER_VER, Compose 版本: $COMPOSE_VER"
}
...
install_portainer() {
    ...
    [ -z "$portainer_dir" ] && echo "❌ 退出安装" && return
...
install_lucky() {
    ...
    [ -z "$lucky_dir" ] && echo "❌ 退出安装" && return
    mkdir -p "$lucky_dir"

    local image="gdy666/lucky:v2"
...
check_backup_deps() {
    ...
    echo "📦 检测到缺少备份所需工具: ${missing[*]}，正在自动安装..."
    case "$OS" in
        ubuntu|debian|armbian)
            apt update
...
check_runlike() {
    if ! docker image inspect wuyangdaily/runlike:latest &>/dev/null; then
        ...
        docker pull wuyangdaily/runlike:latest || { echo "❌ 拉取 runlike 镜像失败"; return 1; }
    fi
    return 0
}
...
EOF
    ...
    for container in "${containers_ref[@]}"; do
        ...
        local run_cmd
        run_cmd=$(docker run --rm -v /var/run/docker.sock:/var/run/docker.sock wuyangdaily/runlike "$container" 2>/dev/null)
        if [ -z "$run_cmd" ]; then
            echo "❌ 无法为容器 $container 生成启动命令，跳过" >&2
            continue
        fi
        local clean_cmd
...
backup_all_containers() {
    ...
    for name in "${all_containers[@]}"; do
        local status=$(docker ps -a --filter "name=^/${name}$" --format "{{.Status}}" | head -n1)
        local image=$(docker ps -a --filter "name=^/${name}$" --format "{{.Image}}" | head -n1)
        if [[ "$status" == Up* ]]; then
            status="🟢 运行中"
        elif [[ "$status" == Exited* ]]; then
            status="🔴 已停止"
...
restore_backup() {
    ...
    echo "📂 解压备份文件..."
    unzip -q "$backup_path" || { echo "❌ 解压失败"; cd - >/dev/null; rm -rf "$restore_root"; return 1; }

    local data_tar="docker_data.tar.gz"
    local start_script="docker_run.sh"

    if [ ! -f "$data_tar" ] || [ ! -f "$start_script" ]; then
        echo "❌ 备份文件缺少必要组件 (data.tar.gz 或 run.sh)"
        cd - >/dev/null
        rm -rf "$restore_root"
        return 1
    fi
...
new_compose() {
    local compose_cmd
    compose_cmd=$(get_compose_cmd) || { echo "❌ 未找到 docker compose / docker-compose"; return; }
    ...
    mkdir -p "$project_dir" || { echo "❌ 无法创建路径 $project_dir"; return; }
    ...
    if ! command -v nano &>/dev/null; then
        ...
        case "$OS" in
            ...
            alpine) apk add nano ;;
            *) echo "❌ 无法自动安装 nano，请手动安装后重试"; return ;;
        esac
        echo "✅ nano 安装完成"
    fi
...
update_compose_all() {
    local compose_cmd
    compose_cmd=$(get_compose_cmd) || { echo "❌ 未找到 docker compose / docker-compose"; pause_return; return; }
    ...
    if [ ${#files[@]} -eq 0 ]; then
        echo "❌ 未在 /root 下找到任何 docker-compose 文件"
        pause_return
        return
    fi
    ...
    for idx in "${selected_indices[@]}"; do
        ...
        echo "🔄 正在更新: $dir"
        cd "$dir" || continue
        $compose_cmd -f "$file" pull || { echo "❌ 拉取镜像失败: $dir"; cd "$original_dir"; continue; }
        $compose_cmd -f "$file" up -d || { echo "❌ 启动服务失败: $dir"; cd "$original_dir"; continue; }
        echo "✅ 已更新: $dir"
        cd "$original_dir"
    done
            ...
            1)
                if ! command -v nano &>/dev/null; then
                    ...
                    case "$OS" in
                        ...
                        suse) zypper install -y nano ;;
                        alpine) apk add nano ;;
                        *) echo "❌ 无法自动安装 nano，请手动安装后重试"; pause_return; continue ;;
                    esac
                    echo "✅ nano 安装完成"
                fi
                new_compose
...
3)
    ...
    if [[ "$ports" =~ ^[aA]$ ]]; then
        ...
        [[ ! "$yn" =~ ^[Yy]$ ]] && { echo "↩️ 已取消操作"; pause_return; return; }
        ...
        for pid in $(lsof -t -iTCP -sTCP:LISTEN 2>/dev/null); do
            ...
            kill -9 $pid >/dev/null 2>&1 && echo "✅ 已结束 $PROC_NAME (PID=$pid)" || echo "❌ 结束失败 PID=$pid"
        done
    ...
    for port in $ports; do
        echo "📌 端口: $port"
        if command -v docker >/dev/null 2>&1; then
            DOCKER=$(docker ps --format "{{.Names}}\t{{.Ports}}" | grep -F ":$port->")
            if [ -n "$DOCKER" ]; then
                CONTAINER=$(echo "$DOCKER" | awk '{print $1}')
                read -r -p "🐳 Docker 占用, 是否停止容器 $CONTAINER ? (Y/N): " yn
                [[ "$yn" =~ ^[Yy]$ ]] && docker stop "$CONTAINER" >/dev/null 2>&1 && echo "✅ 已停止容器 $CONTAINER（端口释放）"
                continue
            fi
        fi
        ...
        if [ -n "$PID" ]; then
            ...
            if [[ "$yn" =~ ^[Yy]$ ]]; then
                kill $PID 2>/dev/null
                sleep 1
                lsof -i :$port >/dev/null 2>&1 || { echo "✅ 已释放端口 $port"; continue; }
                kill -9 $PID && echo "🔥 已强制释放端口 $port" || echo "❌ 释放失败"
            fi
        else
            echo "未被占用"
        fi
        ...
        case $opt in
            ...
            3)
                case $service in
                    ...
                    lucky) uninstall_docker_service "lucky" "Lucky" "lucky" "gdy666/lucky:v2" ;;
...
install_caddy() {
    ...
    TMP_DIR=$(mktemp -d)
    cd "$TMP_DIR" || return 1
    curl -fsSL "$DOWNLOAD_URL" -o caddy.tar.gz || { echo "❌ 下载失败"; cd -; rm -rf "$TMP_DIR"; return 1; }
...
stop_caddy() {
    systemctl stop caddy && echo "✅ Caddy 已停止"
}

restart_caddy() {
    systemctl restart caddy && echo "✅ Caddy 已重启"
}
