...
get_server_ips() {
    ...
    if command -v ip &>/dev/null; then
        mapfile -t ips < <(ip -4 addr show | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | grep -v 127.0.0.1)
...
protect_runlike_container() {
    local protect_container="tmp-runlike-protector"
...
backup_containers_menu() {
    ...
    if [[ "$sel" =~ ^[Aa]$ ]]; then
        ...
        if backup_containers_core all_names "$zip_name"; then
            echo "💡 恢复方法：将文件传输到目标服务器 /root 目录，在本工具中选择「恢复 Docker 容器」即可"
        else
            echo "⚠️ 备份过程中出现错误"
        fi
        return 0
...
update_selected_containers() {
    ...
    for name in $selected; do
        ...
        if docker inspect -f '{{ index .Config.Labels "com.docker.compose.project"}}' "$name" 2>/dev/null | grep -q .; then
        ...
        local new_id=$(docker inspect -f '{{.Id}}' "$image" 2>/dev/null)
    ...
    for d in "${delete_binds[@]}"; do
        if [ -d "$d" ]; then
            ...
            while true; do
                ...
                case "$parent_dir" in
                    /|/bin|/boot|/dev|/etc|/home|/lib|/lib64|/opt|/proc|/root|/run|/sbin|/sys|/tmp|/usr|/var) break ;;
                esac
...
install_1panel() {
    ...
    [[ ! "$confirm" =~ ^[Yy]$ ]] && echo "已取消" && return
    curl -fsSL https://resource.fit2cloud.com/1panel/package/v2/quick_start.sh -o /tmp/quick_start.sh
...
uninstall_1panel() {
    ...
    if [ "$need_manual_help" = true ]; then
        echo "💡 如需手动卸载，请参考以下命令："
        echo "  systemctl stop 1panel"
        echo "  rm -rf /usr/local/1panel /etc/1panel /var/lib/1panel /opt/1panel"
        echo "  rm -f /etc/systemd/system/1panel.service"
...
if [ -z "$LATEST_VERSION" ] || [ "$LATEST_VERSION" = "null" ]; then
    echo "⚠️ 获取最新版本失败，使用默认版本 v2.11.4"
...
update_caddy() {
    ...
    if systemctl reload caddy 2>/dev/null; then
        ...
        echo "⚠️ 热重载失败，尝试完全重启..."
        systemctl restart caddy && echo "✅ 更新完成，Caddy 已重启" || echo "⚠️ 重启失败，请手动检查"
    fi
}
...
export_container_start_cmd() {
    ...
    local container=$(select_containers single "📦 请选择要导出命令的容器：") || { pause_return; return; }
...
set_docker_mirror() {
    ...
    if [ "$has_other" = false ] && [ "$has_custom" = true ]; then
        ...
        if [ ${#final_mirrors[@]} -gt 1 ]; then
            final_mirrors=($(printf '%s\n' "${final_mirrors[@]}" | sort -u))
        fi
    fi

    if [ ${#final_mirrors[@]} -eq 0 ]; then
        echo "⚠️ 没有有效的镜像地址，配置未更改"
        pause_return
        return
    fi
