...
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| [Sun-panel](#sun-panel) | [NginxProxyManager](#nginxproxymanager) | [Portainer](#portainer) | [Lucky](#lucky) | [Cloudreve](#cloudreve) | [Wxchat](#wxchat) | [Wechat](#wechat) | [Komari](#komari-主题) |
| [Agent](#agentuninstall) | [Moments-blog](#moments-blog) | [Mihomo](#mihomo-配置) | [Sub-Store](#sub-store订阅转换) | [Miaospeed](#miaospeed测速后端) | [Koipy](#koipy机器人配置) | [RustDesk](#rustdesk) | [DockerCopilot](#dockercopilot) |
| [配置文件](#配置文件) |  |  |
...
[![GitHub release](https://img.shields.io/github/v/release/jxxghp/MoviePilot)](https://github.com/jxxghp/MoviePilot/releases) [![Docker Pulls](https://img.shields.io/badge/dynamic/json?url=https://hub.docker.com/v2/repositories/jxxghp/moviepilot-v3&query=$.pull_count&label=下载次数&logo=docker)](https://hub.docker.com/r/jxxghp/moviepilot-v3)
```bash
docker run -d \
  --name moviepilot \
  -p 3000:3000 \
...
[![GitHub release](https://img.shields.io/github/v/release/NginxProxyManager/nginx-proxy-manager)](https://github.com/NginxProxyManager/nginx-proxy-manager/releases) [![Docker Pulls](https://img.shields.io/badge/dynamic/json?url=https://hub.docker.com/v2/repositories/jc21/nginx-proxy-manager&query=$.pull_count&label=下载次数&logo=docker)](https://hub.docker.com/r/jc21/nginx-proxy-manager) [![Docker Pulls](https://img.shields.io/badge/dynamic/json?url=https://hub.docker.com/v2/repositories/wuyangdaily/nginx-proxy-manager&query=$.pull_count&label=下载次数&logo=docker)](https://hub.docker.com/r/wuyangdaily/nginx-proxy-manager)
```bash
docker run -d \
  --name NPM \
  -p 80:80 \
  ...
  --restart=always \
  wuyangdaily/nginx-proxy-manager:latest
```
...
## [Komari](https://github.com/nuomiiiii/Lite) [主题](https://github.com/shanyang242/Komari-Theme-LuminaPlus)
[![GitHub release](https://img.shields.io/github/v/release/nuomiiiii/Lite)](https://github.com/nuomiiiii/Lite/releases) [![GitHub release](https://img.shields.io/github/v/release/shanyang242/Komari-Theme-LuminaPlus)](https://github.com/shanyang242/Komari-Theme-LuminaPlus/releases)
```bash
docker run -d \
  --name komari \
  -p 25774:25774 \
...
## [Mihomo](https://github.com/MetaCubeX/mihomo) [配置](https://lanzoux.com/iT9XQ3zqqqhe)
...
- 更新配置：`curl -X PUT http://127.0.0.1:9090/configs -H "Authorization: Bearer 123456" -d "{}"`
```bash
docker run -d \
  --name mihomo \
...
## [Koipy](https://github.com/koipy-org/koipy)（机器人)[配置](https://lanzoux.com/imh7l3m57eyb)
...
## [DockerCopilot](https://github.com/ifsherlock/dockerCopilot)
[![GitHub release](https://img.shields.io/github/v/release/ifsherlock/dockerCopilot)](https://github.com/ifsherlock/dockerCopilot/releases) [![Docker Pulls](https://img.shields.io/badge/dynamic/json?url=https://hub.docker.com/v2/repositories/jaysherlock/dockercopilot&query=$.pull_count&label=下载次数&logo=docker)](https://hub.docker.com/r/jaysherlock/dockercopilot)
```bash
docker run -d \
  --name dockercopilot \
  -v ~/dockercopilot:/data \
  -v /var/run/docker.sock:/var/run/docker.sock \
  -e HTTP_PROXY=http://127.0.0.1:7890 \
  -e HTTPS_PROXY=http://127.0.0.1:7890 \
  -e secretKey=password \
  --network=host \
  --restart=always \
  --privileged=true \
...
slaveConfig: # 后端配置
  ...
  slaves: # 后端列表，注意是数组类型
    - type: miaospeed # 固定值，目前只这个支持
      ...
      mwss: false # 启用双向TLS WebSocket客户端证书认证，默认false，需注意必须同时tls=true，否则启动会报错
