...
## [Komari](https://github.com/komari-monitor/komari) [主题](https://github.com/stqfdyr/komari-theme-Lumina)
[![GitHub release](https://img.shields.io/github/v/release/komari-monitor/komari)](https://github.com/komari-monitor/komari/releases) [![GitHub release](https://img.shields.io/github/v/release/stqfdyr/komari-theme-Lumina)](https://github.com/stqfdyr/komari-theme-Lumina/releases)
```bash
docker run -d \
  --name komari \
  -p 25774:25774 \
...
- 用户名：`admin`
- 密码：`Strong1passwd!`
- 创建网络：`docker network create moments`
```bash
docker run -d \
  --name moments-blog \
  -p 5201:80 \
...
webapi: # Web 配置管理面板（可选）
  enable: true # 是否启用内置 Web 配置 API 服务，默认 false
...
subconverter: # 订阅转换对接配置，可以把base64格式转换bot测试需要的Clash格式
  enable: true # 是否启用
  mode: substore # 可选 subconverter / substore，仅用于推断默认 Host/Port/Target
  template:
