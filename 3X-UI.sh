...
setup_ssl_certificate() {
    ...
    echo -e "${yellow}注意：80 端口必须开放并能从互联网访问${plain}"
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}请确保 80 端口开放，稍后可使用 x-ui 重试${plain}"
...
# 签发 Let's Encrypt IP 证书（短有效期约 6 天）
...
setup_ip_certificate() {
    ...
    # 设置重载命令用于自动续期（添加 || true 避免第一次安装时失败）
    ...
    while true; do
        if is_port_in_use "${WebPort}"; then
            ...
            local alt_port=""
            read -rp "请输入另一个用于 acme.sh 独立验证的端口（留空则退出）: " alt_port
            alt_port="${alt_port// /}"
            if [[ -z "${alt_port}" ]]; then
    ...
    # 安装证书
    # 注意：acme.sh 可能在重载命令失败时报错并退出非零，但证书文件仍会被安装。我们检查文件而非退出码。
    ~/.acme.sh/acme.sh --installcert -d ${ipv4} \
        --key-file "${certDir}/privkey.pem" \
        --fullchain-file "${certDir}/fullchain.pem" \
        --reloadcmd "${reloadCmd}" 2>&1 || true

    # 验证证书文件是否存在（不依赖退出码 - 重载命令失败会导致非零）
    ...
    echo -e "${green}证书有效期约 6 天，会通过 acme.sh cron 任务自动续期。${plain}"
    echo -e "${yellow}acme.sh 将在过期前自动续期并重载 x-ui。${plain}"
    return 0
}

# 全面的手动 SSL 证书签发（通过 acme.sh）
ssl_cert_issue() {
    ...
    if ~/.acme.sh/acme.sh --list 2> /dev/null | awk '{print $1}' | grep -Fxq "${domain}"; then
        ...
        echo -e "${yellow}发现已存在 ${domain} 的证书，将复用该证书。${plain}"
    ...
    if [[ ${WebPort} -gt 65535 || ${WebPort} -lt 1 ]]; then
        echo -e "${yellow}您输入 ${WebPort} 无效，将使用默认端口 80。${plain}"
        WebPort=80
    fi
    echo -e "${green}将使用端口: ${WebPort} 签发证书。请确保此端口开放。${plain}"
    ...
    echo -e "${green}该命令将在每次证书签发和续期时执行。${plain}"
    ...
    if [[ "$setPanel" == "y" || "$setPanel" == "Y" ]]; then
        ...
        if [[ -f "$webCertFile" && -f "$webKeyFile" ]]; then
            ...
            echo -e "${red}错误：未找到域 ${domain} 的证书或私钥文件。${plain}"
        fi
    else
...
prompt_and_setup_ssl() {
    ...
    echo -e "${green}3.${plain} 自定义 SSL 证书（使用现有证书文件路径）"
    echo -e "${green}4.${plain} 跳过 SSL（高级用法——位于反向代理或 SSH 隧道之后）"
    echo -e "${blue}注意：${plain} 选项 1 和 2 需要 80 端口开放。选项 3 需要手动提供路径。"
    echo -e "${blue}注意：${plain} 选项 4 将通过普通 HTTP 提供面板服务——仅当位于 nginx/Caddy 或 SSH 隧道之后才安全。"
    read -rp "请选择一个选项（默认 2 使用 IP）: " ssl_choice
    ssl_choice="${ssl_choice// /}" # 去除空格

    # 如果输入为空或无效（不是 1、3、4），默认选择 2（IP 证书）
    ...
    case "$ssl_choice" in
        ...
        2)
            ...
            echo -e "${green}使用 Let's Encrypt IP 证书（短有效期配置）...${plain}"
        ...
        4)
            echo ""
            echo -e "${red}⚠ 面板将安装而不启用 SSL/TLS。${plain}"
            ...
            if [[ "$bind_local" == "y" || "$bind_local" == "Y" ]]; then
                ...
                SSL_HOST="127.0.0.1"
                echo -e "${green}✓ 面板已绑定到 127.0.0.1，无法从公网访问。${plain}"
                echo ""
                echo -e "${green}SSH 端口转发 — 从本地计算机打开面板的方法：${plain}"
                echo -e "  标准 SSH 命令："
                ...
                echo -e "${yellow}面板将在所有接口上监听普通 HTTP。请确保前面有其他东西终止 TLS。${plain}"
            fi
...
config_after_install() {
    ...
    if [[ -z "$server_ip" ]]; then
        echo -e "${yellow}无法从任何服务提供商自动检测服务器 IP。${plain}"
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            read -rp "是否要自定义面板端口设置？（如果不，将应用随机端口）[y/n]: " config_confirm
            ...
            echo ""
            echo -e "${green}═══════════════════════════════════════════${plain}"
            echo -e "${green}     SSL 证书设置（强烈推荐）              ${plain}"
            echo -e "${green}═══════════════════════════════════════════${plain}"
            ...
            echo -e "${green}Web 路径:  ${config_webBasePath}${plain}"
            ...
            local config_webBasePath=$(gen_random_string 18)
            echo -e "${yellow}Web 路径缺失或太短，正在生成新的...${plain}"
            ${xui_folder}/x-ui setting -webBasePath "${config_webBasePath}"
            echo -e "${green}新的 Web 路径: ${config_webBasePath}${plain}"

            # 如果面板已安装但未配置证书，则提示 SSL 设置
        ...
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            echo -e "${green}用户名、密码和 Web 路径已正确设置。${plain}"
        fi
...
install_x-ui() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        # 如果在 tar.gz 中未找到服务文件，从 GitHub 下载
        if [ "$service_installed" = false ]; then
        ...
        if [ "$service_installed" = true ]; then
            echo -e "${green}设置 systemd 单元...${plain}"
